...
describe('reading from the store', () => {
  ...
  it("does not canonicalize custom scalar objects", function () {
    ...
    // important that it be the very same (that is, ===) Date object that
    // was returned from the read function for the Query.now field, not a
    // canonicalized version.
    expect(result1.now).toBe(now);
    expect(result2.now).toBe(now);

    // However, since the Query.abc field returns a "normal" object, it
    // gets canonicalized, which guarantees result1.abc === result2.abc,
    // but that object is !== abc, because of the canonicalization.
    expect(result1.abc).toEqual(abc);
    expect(result2.abc).toEqual(abc);
    expect(result1.abc).toBe(result2.abc);
    expect(result1.abc).not.toBe(abc);
    expect(result2.abc).not.toBe(abc);
  });
});
